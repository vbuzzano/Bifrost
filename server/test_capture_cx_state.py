"""Tests for capture.py's CX (commodity) state tracking.
...
def test_set_amiga_cx_state_enabled_clears_disabled_flag():
    capture._amiga_cx_disabled = True
    capture.set_amiga_cx_state(True)
    assert capture._amiga_cx_disabled is False


def test_set_amiga_cx_state_disabled_sets_flag():
    capture._amiga_cx_disabled = False
    capture._focus = capture.FOCUS_PC  # avoid spawning a focus-change thread
    capture.set_amiga_cx_state(False)
    assert capture._amiga_cx_disabled is True


def test_reset_amiga_cx_state_clears_flag():
    capture._amiga_cx_disabled = True
    capture._reset_amiga_cx_state()
    assert capture._amiga_cx_disabled is False


def test_set_focus_amiga_refused_while_cx_disabled():
